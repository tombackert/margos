...
- [x] SRQ2: Run 10 interleaved trials (Manual, Margos, Manual, Margos...) per protocol order
...
- [x] SRQ3: Run reference experiment: `margos run aggregation_srq3` (~6 min)
...
**Margos Commit:** `06eb747` — all Margos and experiment setup changes frozen at this state.
This is the exact Margos version used for all SRQ data collection.
...
- [x] SRQ5: Prepare shareable experiment: `margos export <aggregation_v1_ref_id>`
- [x] SRQ5: Run 20 interleaved trials (10 manual + 10 Margos)
- [x] SRQ5: For each Margos trial: audit bundle completeness → fill `bundle_audits.csv`
...
**Result:** H5 partially supported. The recorded SRQ5 handoff trials show that Steps-to-Share fell by 87.5%, Time-to-Share by 65.3%, and Time-to-Reproduce by 45.9%, while Handoff-Success-Rate remained 100% in both conditions. Time-to-First-Run increased by 7.4% in Margos condition because import overhead still outweighed the direct file-copy path in the prepared-collaborator setup.

## Completed — SRQ4: Usability

- [x] SRQ4 (Part 1): Fill `heuristic_audit.md` 35-criteria checklist against live Margos; compute Heuristic-Compliance-Rate
- [x] SRQ4 (Part 2): Fill `klm_analysis.md` for 7 tasks (baseline + Margos); compute KLM-Reduction
...
- [x] Update Protocol-SRQ2: `margos report` → `margos compare`
- [x] Update Protocol-SRQ3: `margos report exp_XXX --reference results/reference/` → `margos compare exp_XXX results/reference/`
- [x] Update Protocol-SRQ5: `margos run --verify-reference` → `margos run` + `margos compare` (two steps)
...
- [x] Cleaned up training script - Moved Ray logging utilities to `margos/utils/ray_logging.py`, simplified `aggregation.py` to pure training logic
...
- [x] Reference experiment selection: Use `margos report --compare` for interactive reference selection
...
- [x] Show usage explainer as: "Usage: margos [run | report | export | import]"
...
- [x] Need command margos show config/experiments/results -> Added `margos show [configs|results|imported|bundles|all]`
