...
The final SRQ5 comparison rule uses the mean of the last 50 reward values with a ±1% threshold for the reported SRQ5 handoff status. `margos compare` also exposes a stricter SRQ3 reproducibility status, but that stricter judgment is not part of M5.5. This threshold tightening does not change the observed SRQ5 outcomes because all recorded Margos handoffs showed 0% deviation from the reference.
